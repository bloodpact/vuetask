...
<script>
  ...
  export default {
    ...
    methods:{
        checkStat(){
          let calendarStart = this.dataReq.dateFrom;
          let calendarEnd = new Date(calendarStart);
          let newEnd =  calendarEnd.setDate(calendarEnd.getDate()+30);
          let alldates = [];

          for (let i=0; calendarStart <= calendarEnd; calendarStart.setDate(calendarStart.getDate() + 1), i++) {
            alldates.push(new Date(calendarStart));
          }
          // в объекте по умолчанию меньше данных чем в массиве дат поэтому undefined
